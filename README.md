...
| **2 — Universal isolation** | **Docker Sandboxes (`sbx`) — our default recommendation for anyone with Docker** ([start here](#start-here-docker-sandboxes-sbx)); also the `srt` wrapper, and Apple `container` microVMs as an emerging option | **All three tools' CLIs**, including agents with no built-in sandbox. Not in-IDE agents: Copilot agent mode in JetBrains still has no sandboxed option | [docker-sandbox](docs/docker-sandbox.md) · [srt](docs/universal-sandbox-srt.md) · [apple-container](docs/apple-container.md) |
...
Tiers compose: a developer on Tier 1 today already has a boundary in place (as strong as that tier actually holds — see the warning above); Tier 3 makes sure it stays on; Tier 2 covers the CLI tools that have no built-in story (in-IDE agents without a built-in sandbox, like Copilot in JetBrains, remain a gap).
...
- **Copilot CLI** → run `/sandbox enable` in a session (public preview); for VS Code set [`configs/copilot/vscode-settings.json`](configs/copilot/vscode-settings.json). **Copilot agent mode in JetBrains has no sandbox and no sandboxed alternative — use Copilot CLI with `/sandbox enable` instead, or disable IDE agent mode via org policy.** [Guide](docs/copilot.md)
- **Any tool — whole-process isolation** → [Docker Sandboxes](docs/docker-sandbox.md) (`sbx`) is our default recommendation when Docker is available — see [Start here](#start-here-docker-sandboxes-sbx) above. Without Docker, wrap the CLI in [`srt`](docs/universal-sandbox-srt.md). Neither puts an agent that runs *inside* your IDE behind a boundary — keep the IDE on the host and run the agent's CLI in the sandbox ([details](docs/docker-sandbox.md#using-it-with-an-ide-vs-code-jetbrains)).
