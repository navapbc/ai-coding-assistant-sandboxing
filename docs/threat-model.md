...
| Default-deny egress | Only allowlisted domains reachable — **but for Claude Code this holds only under the strict posture** (`allowManagedDomainsOnly: true`) or when a human denies the prompt; under the standard posture an auto-allowed/agent session reaches *unlisted* domains unprompted ([decision](enforcement.md#the-strict-vs-standard-domain-decision)). Docker Sandboxes (`balanced` preset) and the `srt` proxy are unconditional default-deny. | Filtering proxy outside the sandbox (built-in tools, `srt`, Docker Sandboxes' host proxy) |
