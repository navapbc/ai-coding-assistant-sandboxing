...
3. **IP-resolved firewalls drift.** The devcontainer firewall resolves domains to IPs at start; CDN rotation can break (or, with broad ranges, over-permit) later. Hostname-aware proxy filtering is tighter; the container approach trades that for uniform coverage.
