...
3. **IP-resolved firewalls drift.** The devcontainer firewall resolves domains to IPs at start; CDN rotation can break (or, with broad ranges, over-permit) later. Hostname-aware filtering doesn't drift — the built-in proxy tiers and **Docker Sandboxes** (which TLS-*terminates*) already match by hostname, and the devcontainer ships an experimental opt-in [SNI-proxy egress mode](../configs/devcontainer/egress-proxy/README.md) (Envoy, hostname-matched, no IP resolution) for teams that want the same on the container tier. SNI matching still doesn't defeat domain fronting or ECH (see #2) — only TLS termination does — so the default IP firewall trades hostname precision for simple, uniform, no-MITM coverage.
