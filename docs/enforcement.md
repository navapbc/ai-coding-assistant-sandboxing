...
| `allowManagedDomainsOnly: false` (shipped default) | Standard posture: the managed `allowedDomains` are the baseline, and projects can *add* domains via reviewed PRs; an unlisted domain **prompts** the developer — **but only when a human is present to answer it** (see the caveat below). Set to `true` for the strict posture, which hard-blocks unlisted domains with no prompt — **required for unattended/agent use** |
...
The shipped managed file uses the **standard** posture (`allowManagedDomainsOnly: false`): an unlisted domain **prompts** rather than hard-blocking, and a project can add a domain through a reviewed one-line PR to its `.claude/settings.json` — same business day, no MDM cycle.

> [!WARNING]
> **The standard posture only preserves default-deny when a human is at the keyboard.** That "prompt" is a *human* approval step. In an **auto-allowed** session (`autoAllowBashIfSandboxed: true` — our own baseline), or any headless/unattended **agent** run, there is no one to answer it — and we have observed unlisted domains (e.g. `cms.gov`, `example.com`) reaching the network with **no prompt and no block** in that configuration. So under standard posture an autonomous agent's egress is **not** default-deny — it can reach arbitrary domains. Since a hijacked autonomous agent is exactly this repo's [threat model](threat-model.md), **use the strict posture (`allowManagedDomainsOnly: true`) for any unattended or agent use.** Don't assume — confirm with the [egress check](troubleshooting.md#verify-your-egress-is-actually-default-deny).

The **strict** posture (`allowManagedDomainsOnly: true`, honored from managed settings only) makes only the managed `allowedDomains` count: every addition — including per-project registries — goes through an MDM push, and unlisted domains **hard-block** instead of prompting. The trade-off is real (no same-day per-project domain PR; an MDM cycle for every addition), but it is the only posture that contains an autonomous agent. **Recommended default for fleet and agent use; the standard posture is acceptable only where a human answers every egress prompt.** Whichever you pick, make it a recorded decision rather than a drift, and re-run the egress check after deploying.
