...
**Legend:** ✅ **sandbox available** — a boundary can be configured (link shows how) · ⚠️ **conditional** — only under a stated constraint (inside Docker Sandboxes only, per-session, …) · ❌ **no sandbox** — no boundary available for this surface today, or a mode that removes it.
...
| Claude Code with MCP servers | ⚠️ Conditional | MCP servers run **unsandboxed** on the host → sandbox only in [Docker Sandboxes](docker-sandbox.md), with each server reviewed like a dependency |
...
| Copilot CLI, no sandbox | ❌ No sandbox | To get a boundary: enable the sandbox, wrap in [`srt`](universal-sandbox-srt.md), or run it in [Docker Sandboxes](docker-sandbox.md) |
...
| Copilot agent mode, JetBrains, on host | ❌ **No sandbox** | No OS sandbox exists for this surface, and this repo offers no sandboxed alternative for in-IDE agent mode. Use Copilot CLI with the sandbox (or in [Docker Sandboxes](docker-sandbox.md)) instead. Org policy can disable IDE agent mode ([enforcement](enforcement.md)) |
...
2. **Wrapper and sandbox-launcher tiers aren't intercepting.** `srt` / `run-sandboxed.sh` / `sbx run` protect only the command launched *through* them — nothing stops a bare `claude`/`copilot`. For Claude Code and Codex, managed settings make the protection travel with the tool; the wrapper approaches have no OS-level "always intercept," so in a locked-down setup they have to be the only sanctioned entry point.

Net: **Claude Code and Codex can be genuinely default-on and enforced — including in IntelliJ and VS Code — via managed settings.** Copilot cannot be made default-on across the board today (per-session CLI toggle, no JetBrains sandbox), which is why Copilot-in-IntelliJ routes to Copilot CLI and the org-level "disable IDE agent mode" policy covers surfaces you can't sandbox.

## Cross-cutting requirements (all tools)

| Requirement | Where enforced |
|-------------|----------------|
| Default-deny egress; allowlist per [network-allowlists.md](network-allowlists.md) | Tool sandbox config / srt / Docker Sandboxes policy |
| No cloud-provider storage domains, ever | Allowlist review; managed domain locks |
| Secret paths unreadable (`~/.ssh`, `~/.aws`, `~/.kube`, keychain, …) | `denyRead` / Seatbelt profile / microVM boundary (host home not mounted) |
