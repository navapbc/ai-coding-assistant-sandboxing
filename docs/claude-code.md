...
**Status: sandbox available** on all surfaces with the configuration below (experimental — verify it holds for yourself). See [policy-matrix.md](policy-matrix.md).
...
- **`sandbox.enabled: true`, `autoAllowBashIfSandboxed: true`** — every Bash command runs inside Seatbelt, auto-approved on the premise that the boundary contains it.
...
- **`filesystem.denyRead`** — blocks the credential paths `~/.ssh`, `~/.aws`, `~/.azure`, `~/.config/gcloud`, `~/.kube`, `~/.gnupg`, `~/.netrc`, `~/.npmrc`, `~/.pypirc`, `~/.docker`, `~/Library/Keychains`, plus `~/.config/sops` (SOPS age keys) and the shell histories `~/.bash_history`/`~/.zsh_history` (which often hold pasted tokens). **This matters because the sandbox default allows reading your whole disk** — write scope is narrow by default, read scope is not. `denyRead` accepts **literal path prefixes only — no globs**, so it can't express file-*type* denials like `*.key`; those go in `permissions.deny` instead (next bullet). (`~/.gitconfig` is deliberately *not* on this list: git needs it for commit identity, and its write is already blocked, which is what closes the git-based credential-helper injection vector — just don't store tokens in it.)
- **`permissions.deny` `Read(…)`** — gitignore-style globs that the OS-level `denyRead` can't express: `Read(**/*.key)` (private keys) and `Read(**/*.sw[a-p])` (vim swap files, which can hold unsaved buffer contents of a secret file), alongside the home-secret and `./.env` / `./secrets/**` rules. These are **tool-layer** checks: a `Read` deny also blocks Edit/Write and the file-reading Bash commands Claude Code recognizes (`cat`, `head`, `tail`, `sed`), but **not** an arbitrary subprocess that opens the file itself (e.g. a Python script) — only the OS sandbox catches that, and it can't glob. So `*.key`/swap protection is best-effort at the tool layer here; for OS-level denial of those file types use the [universal wrapper](universal-sandbox-srt.md), whose Seatbelt profile denies them by regex.
...
- **`env`** — `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1` (no Statsig/Sentry telemetry, so those domains never need allowlisting) and `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=1`. The latter is **documented** to strip Anthropic and cloud-provider credentials from Bash subprocesses ([per the sandboxing docs](https://code.claude.com/docs/en/sandboxing)) — but it is a **fixed built-in heuristic, not a configurable allowlist**, its exact variable set is undocumented, and **we have only partially verified it**: we confirmed it does **not** cover GitHub PATs (a sandboxed `echo $GITHUB_TOKEN` returns the token in full), and we have **not** confirmed it actually strips any specific AWS/GCP/Azure variable. So treat the cloud-credential scrubbing as an **unverified vendor claim**, not a guarantee — and don't rely on it at all for `GITHUB_TOKEN`/`GH_TOKEN` or other non-cloud secrets. The OS sandbox doesn't help here either: it confines filesystem and network, not the inherited environment. Unlike Claude Code's fixed heuristic, [Codex exposes a *configurable* deny-by-default env policy](codex.md#environment-variables-dont-trust-the-defaults); the equivalent here is the `env -i` allowlist in [`configs/seatbelt/run-sandboxed.sh`](../configs/seatbelt/run-sandboxed.sh), which applies to commands you launch through that wrapper — not to Claude Code's own Bash tool. The durable fix is to keep long-lived secrets out of your shell environment entirely.
