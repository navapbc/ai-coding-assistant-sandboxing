...
### `pip install` / `npm install` fails inside the sandbox

Almost always a **write-scope** problem, **not** a network one — the package registries (`pypi.org`, `files.pythonhosted.org`, `registry.npmjs.org`, Maven, Gradle, Rust, NuGet, Yarn, Ruby) are already in the [default allowlist](network-allowlists.md#per-stack-package-registries). The sandbox's write scope is **the workspace + `/tmp`**, so an installer fails when it writes *outside* it: a global `pip install` into system `site-packages`, `pip install --user` into `~/.local` or `~/Library/Python`, or a cache under `~/.cache/pip` / `~/.npm`. Fix it by keeping the writes inside the workspace, in this order:

1. **Install into the workspace.** A Python `venv` created in the repo, `node_modules/`, Cargo's `target/`, etc. all sit inside the write scope and just work: `python3 -m venv .venv && .venv/bin/pip install …`. This is the right answer for almost every case.
2. **Redirect caches into the workspace or `/tmp`.** `PIP_CACHE_DIR=$PWD/.pip-cache` (or `/tmp/pip-cache`), `npm config set cache /tmp/npm-cache`, `CARGO_HOME=$PWD/.cargo`, etc.
3. **Only if a tool genuinely needs a specific home-dir path**, add *that one path* narrowly (`sandbox.filesystem.allowWrite: ["~/.cache/<tool>"]`) — never `~/`, and never a whole home subtree you don't understand.

> [!WARNING]
> **Do not "fix" this by adding `pip`, `npm`, `uv`, `poetry`, `cargo`, etc. to `excludedCommands`.** That runs the installer **completely unsandboxed** — full home-directory read *and* open network — so a single malicious dependency (a `postinstall` script, a `setup.py`, a build hook) can read `~/.ssh` and exfiltrate it. Installers are the *highest-value* thing to keep sandboxed, not the first thing to exclude. `excludedCommands` is a hole in the wall with no managed-scope lock and should show up as a [code-review flag](enforcement.md#single-machine-solo-developer-no-mdm) ([threat model](threat-model.md)); an installer belongs there least of all.
...
**What `enableWeakerNetworkIsolation: true` actually does — and doesn't.** It relaxes the sandbox's **TLS certificate verification** so processes trust a custom CA (the case that matters: a corporate MITM proxy set via `httpProxyPort` whose CA the sandbox otherwise won't honor). It does **not** disable the hostname/`allowedDomains` egress allowlist — default-deny egress stays enforced; only the TLS-trust check is loosened, which is why it's a narrower, safer lever than `excludedCommands` (which drops the whole boundary). The trade-off is real, though: you're shifting the "reject a bad certificate" guarantee from the sandbox onto your corporate proxy, so use it **only** where that proxy genuinely exists and is trusted. Keep it **off by default** — it's an unlocked key with no managed-scope lock, so like `excludedCommands` it's a [CI-grep / code-review flag](enforcement.md#single-machine-solo-developer-no-mdm) if it turns up in a repo's settings. (Don't confuse it with `enableWeakerNestedSandbox`, a Linux/Docker-only `/proc` setting — unrelated.)
